    ...
    [self.ff_allItem enumerateObjectsUsingBlock:^(__kindof FFView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        switch (self.layoutDirection) {
            ...
            case FFContainerViewLayoutDirectionHorizontal:{
                // 判断宽度是否还可以放得下当前的视图
                useWidth = lastView ? (lastView.ff_right + lastView.marginInsets.right) : self.paddingInsets.left; /**< 目前已使用的宽度 */
                CGFloat remainingWidth = self.ff_width - useWidth - obj.marginInsets.left - obj.marginInsets.right - self.paddingInsets.right;
                
                width = obj.size.width - obj.marginInsets.left - obj.marginInsets.right;
                if (width <= 0.f) {
                    width = self.ff_width - self.paddingInsets.left - self.paddingInsets.right - obj.marginInsets.left - obj.marginInsets.right;
                }
                if (remainingWidth >= width && lastView) {
                    // 可以放得下
                    x = useWidth + obj.marginInsets.left;
                    y = lastView.ff_y - lastView.marginInsets.top + obj.marginInsets.top;
                } else {
                    // 放不下
                    x = self.paddingInsets.left + obj.marginInsets.left;
                    ...
                    if (lastView) {
                        lastView.ff_width = MAX(0.f, lastView.ff_width - self.paddingInsets.right);
                    }
                }
            }
                break;
        ...
        useHeight = MAX(useHeight, obj.ff_bottom + obj.marginInsets.bottom);
        lastView = obj;
    }];
...
- (void)ff_refreshSize {
    ...
    self.ff_height = self.size.height;
